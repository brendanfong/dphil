...
by considering the symplectic vector space $\vect{X}$, the direct sum of the space
$\F^X$ of potentials and the space ${(\F^X)}^\ast$ of currents at these points.
A Lagrangian relation specifies which states of the output space $\vect{Y}$ are
allowed for each state of the input space $\vect{X}$.
...
\[  Sf\maps \vect{B} \to \vect{A} \] 
and `twisted symplectification'
\[  S^tf\maps \vect{B} \to \overline{\vect{A}}\]
...
for any given potential-current pair $(\psi,\iota)$ in $\vect{B}$, its image
under $Sf$ comprises all elements of $(\psi', \iota') \in \vect{A}$ such that
...
$S^tf\maps \vect{B} \to \overline{\vect{A}}$ almost identically to the above, except that we flip the sign of the currents $\iota' \in (\F^A)^\ast$.  We use the twisted symplectification $S^ti$ of the input function to relate the state on $\partial N$
...
\begin{theorem} \label{main_theorem}
  ...
  $\vect{X}$ it generates, and a circuit $\big((N,E,s,t,r),i,o\big)$ to the Lagrangian     
  relation 
  \[
...
Part \ref{part:categories}, on the syntax of circuit diagrams, contains the main
...
\begin{example}[The symplectic vector space generated by a finite set]
  ...
  Given a finite set $N$, we consider the vector space $\vect{N}$ a symplectic
  vector space $(\vect{N},\omega)$, with symplectic form 
  \[
  ...
  $\{(\phi_n,0),(0,i_n)\}_{n\in N}$ forms a symplectic basis for $\vect{N}$.  
\end{example}
...
In the symplectic vector space $\vect{N}$, the subspace $\F^N$ has the
...
subspaces, and they may all be realized as the image of $\vect{N}$ 
under symplectomorphisms from $\vect{N}$ to itself.
...
$N$ give rise to Lagrangian subspaces of $\vect{N}$. More generally, we show
...
\begin{proposition} \label{prop:qfls}
  ...
  subspace 
  \[ 
    L_Q = \big\{(\phi,dQ_\phi) \mid \phi \in \F^N\big\} \subseteq \vect{N},
  \] 
  ...
  \[ 
    ...
    \bigg\{\begin{array}{c} \mbox{Lagrangian subspaces of $\vect{N}$}\\
      \mbox{with trivial intersection with $\{0\} \oplus {(\F^N)}^\ast \subseteq
      \vect{N}$} \end{array} \bigg\}.  
  \]
\end{proposition}
\begin{proof}
  The symplectic structure on $\vect{N}$ and our notation for it is given in
...
given a symplectic vector space $V$, the Lagrangian relation $\id\maps V \to V$
specified by the Lagrangian subspace
\[
  \id = \{(v,v) \mid v \in V\} \subseteq \overline{V} \oplus V,
\]
...
$\vect{X}$.  For morphisms, recall that a function $f\maps X \to Y$ between
finite sets may be considered as a corelation, and the symplectification functor
$S$ thus maps this corelation to some Lagrangian relation $Sf\maps \vect{X}
\to \vect{Y}$. As Lagrangian relations map Lagrangian subspaces to Lagrangian
...
\begin{align*}
  ...
  Q &\longmapsto \{(\phi,dQ_\phi) \mid \phi \in \F^N\} \subseteq \vect{N}.
\end{align*}
...
\[
  \vect{X} \oplus \vect{Y} \stackrel{\sim}{\longrightarrow} \vect{X+Y} 
\]
and
\[
  \{0\} \stackrel{\sim}{\longrightarrow} \vect{\varnothing}.
\]
...
\[
  \xymatrix@C=30pt@R=5pt{
    ...
    \ar[rr]_(.53){\id_X+[\id_Y,\id_Y]+\id_Z} && X+Y+Z
  }
\]
...
\[
  \xymatrix{
    ...
    & X+Y+Y+Z \ar[ul]^{[i_X,o_Y]+[i_Y,o_Z]\qquad} \ar[ur]_{\qquad\id_X+[\id_Y,\id_Y]+\id_Z} 
  }
\]
...
\[
  \xymatrix{
    ...
    & X+Y+Y+Z \ar[ul]^{[i_X,o_Y]+[i_Y,o_Z]\quad} \ar[ur]_{\qquad \id_X+[\id_Y,\id_Y]+\id_Z} 
  }
\]
