...
and the positive real function $Z$ is the \define{impedance} of the component.   As we shall
...
As before, to each passive linear circuit we associate a generalised Dirichlet
form.
...
\]
and we call $\varphi \in \F^N$ a \define{potential}. Note have assumed that
...
may define its \define{conjugate} symplectic form $\overline\omega = - \omega$,
...
$\nu+\omega$, and call this the \define{sum} of the two symplectic vector
...
In this subsection we consider the case of currents, described by a functor
...
We have thus shown that we do indeed have a functor $S\maps \mathrm{Corel} \to
