...
As before, we define two circuits to be \define{equivalent} if they have the
same power functional, and define the \define{behaviour} of a circuit to be its
equivalence class. 
...
functor to a hypergraph category. A third, less formal desideratum is that the
category of behaviours and its composition rule should be easy to define and
work with. 
...
monoidal natural transformation $\alpha\maps (\mathrm{Graph},\rho) \Rightarrow
(\mathrm{Dirich},\delta)$.
...
To construct a category in which all circuits with the same behaviour have the
same image, we wish to map a circuit to its power functional. We will use
decorated corelations to construct such a category. Ideally, we would like to
use an isomorphism-morphism factorisation on $\FinSet$, so that for a map $X \to
Y$ the Dirichlet form decorates the set $X+Y$, and we need not talk at all about
cospans. We saw however, in the previous section, that a category with morphisms
Dirichlet forms on the disjoint union of the domain and codomain is not posible.
Instead recall that $\FinSet$ has an epi-mono factorisation system
$(\mathrm{Sur},\mathrm{Inj})$, with $\mathrm{Inj}$ stable under pushout. We will
construct a category of decorated jointly epic cospans. 
...
obtain the power functional of the circuit. We show that this view of
minimisation allows us to define a decorated corelations category with
jointly epic cospans decorated by power functionals as morphisms, and
composition given by minimising the sum of the power functionals over the new
interior nodes.
