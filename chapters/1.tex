...
Only the `topology' of the diagrams matters: if two diagrams are homotopy
...
\end{tikzpicture}
  \end{aligned}
\]
for all $A,B,C$ in $\mc C$.  We call $\s$ the \define{braiding}.
...
with finite colimits. A \define{cospan} $X \stackrel{i}{\longrightarrow} N
...
Write $\FinSet$ for the category of finite sets and functions. It is well-known,
...
\define{wide} if $\mathcal C$ contains all objects of $\mathcal D$, and call a
functor that is faithful and bijective-on-objects a \define{wide embedding}.
