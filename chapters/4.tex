...
and decorations are the same for all morphisms. So for a decorated cospan
category over the $\FinSet$, the number of morphisms $0 \to 1$ cannot be more
than countably many times those $0 \to 0$; the only difference is a decorated
cospan $0 \to 1$ includes a map from 1 to the apex $N$ that chooses an element
of the apex. But the skeletal category of vector spaces and linear maps over
$\mathbb{R}$ with monoidal product the tensor product has $\mathbb{R}$ morphisms
$0 \to 0$, and $\mathbb{R}^2$ morphisms $0 \to 1$. NEEDS WORK
...
\begin{proof}
  ...
  \]
  This is symmetric lax monoidal functor mapping $X$ to the homset $\mc H(I,X)$,
...
\begin{proof}
  ...
  hypergraph categories. The previous theorem implies there exist symmetric lax
  monoidal functors 
  \[
...
We have a functor $\Set \to \mathrm{Cat}$ taking a set $\mc O$ to the lax
symmetric monoidal functor category $\mathrm{SymLaxMon}(\cospan(\FinSet_{\mc
O}), \Set)$. Note that a function $r\maps \mc O \to \mc O'$ maps to a functor
$\rho\maps \mathrm{SymLaxMon}(\cospan(\FinSet_{\mc O}), \Set) \to
\mathrm{SymLaxMon}(\cospan(\FinSet_{\mc O'}), \Set)$. The Grothendieck
construction gives a category where are objects are some label set $\mc O$
together with an object in $\mathrm{SymLaxMon}(\cospan(\FinSet_{\mc O}),
\Set)$---that is, with a lax symmetric monoidal functor $F\maps
(\cospan(\FinSet_{\mc O}),+) \to (\Set,\times)$. The morphisms $(\mc O, F) \to
(\mc O,F')$ are functions of label sets $r: \mc O \to \mc O'$ together with a
natural transformation $\theta\maps \rho(F) \Rightarrow F'$.

\begin{theorem}
\[
  \mathrm{HypCat} \cong \Set\int
  \mathrm{SymLaxMon}(\cospan(\FinSet_{-}), \Set)
\]
\end{theorem}
...
The above equivalence is the subject of a forthcoming paper \cite{SV}

\subsection{Examples}
We give some examples reproducing hypergraph categories as decorated corelations
categories.

\begin{example}[Empty decorations and equivalence relations]
