...
$1_R$ of the ring $R$.  As in the introduction, we show $R^{(-)}\mathrm{Cospan}$
is the category of `multivalued matrices' over $R$, and $R^{(-)}\mathrm{Corel}$
is the category of matrices over $R$. 

It is the coherence maps $\varphi_{N,M}$ that enact multiplication of scalars,
with the composite of $(X \xleftarrow{i_X} N \xrightarrow{o_Y} Y, N
\xrightarrow{s} R)$ and $(Y \xleftarrow{i_Y} M \xrightarrow{o_Z} Z, M
...
categorical product. We discussed the intuition for this composition rule in the
introduction. 

These morphisms are multivalued matrices in the following sense: the cardinality
of the domain and the codomain give the dimension of the matrix, and the apex
$N$ indexes the entries of the matrix.
...
This becomes a decorated relations example if we extend $R^{(-)}$ to the
